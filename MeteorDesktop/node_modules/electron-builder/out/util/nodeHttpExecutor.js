...
class NodeHttpExecutor extends (_electronBuilderHttp || _load_electronBuilderHttp()).HttpExecutor {
    download(url, destination, options) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            return yield new (_bluebirdLstC2 || _load_bluebirdLstC2()).default(function (resolve, reject) {
                _this.doDownload(url, destination, 0, options || {}, agent, function (error) {
                    if (error == null) {
                        resolve(destination);
                    } else {
                        reject(error);
    ...
    doApiRequest(options, token, requestProcessor) {
        ...
        if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`HTTPS request: ${ JSON.stringify(options, null, 2) }`);
        }
        if (token != null) {
            ...
            const request = (_https || _load_https()).request(options, response => {
                try {
                    if (response.statusCode === 404) {
                        ...
                        reject(new (_electronBuilderHttp || _load_electronBuilderHttp()).HttpError(response, `method: ${ options.method } url: https://${ options.hostname }${ options.path }
...
`));
                    ...
                    response.on("end", () => {
                        try {
                            ...
                            const isYaml = options.path.indexOf(".yml") !== -1;
                            if (response.statusCode >= 400) {
                                if (isJson) {
                                    reject(new (_electronBuilderHttp || _load_electronBuilderHttp()).HttpError(response, JSON.parse(data)));
                                } else {
                                    reject(new (_electronBuilderHttp || _load_electronBuilderHttp()).HttpError(response));
                                }
                            } else {
                                resolve(data.length === 0 ? null : isJson ? JSON.parse(data) : isYaml ? (0, (_jsYaml || _load_jsYaml()).safeLoad)(data) : data);
                            }
                        } catch (e) {
                            reject(e);
                        }
