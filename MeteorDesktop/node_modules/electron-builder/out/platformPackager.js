...
class PlatformPackager {
    ...
    get relativeBuildResourcesDirname() {
        return (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)((0, (_metadata || _load_metadata()).getDirectoriesConfig)(this.info.devMetadata), it => it.buildResources) || "build";
    }
    computeAppOutDir(outDir, arch) {
        return _path.join(outDir, `${ this.platform.buildConfigurationKey }${ (0, (_electronBuilderCore || _load_electronBuilderCore()).getArchSuffix)(arch) }${ this.platform === (_electronBuilderCore || _load_electronBuilderCore()).Platform.MAC ? "" : "-unpacked" }`);
    }
    ...
    pack(outDir, arch, targets, postAsyncTasks) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            const appOutDir = _this.packagerOptions.prepackaged || _this.computeAppOutDir(outDir, arch);
    ...
    getResourcesDir(appOutDir) {
        return this.platform === (_electronBuilderCore || _load_electronBuilderCore()).Platform.MAC ? this.getOSXResourcesDir(appOutDir) : _path.join(appOutDir, "resources");
    }
    ...
    computePublishConfigs(targetSpecificOptions) {
        var _this8 = this;

        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            let publishConfigs = getPublishConfigs(_this8, targetSpecificOptions);
            ...
            if (publishConfigs.length === 0) {
                ...
                const repositoryInfo = yield _this8.info.repositoryInfo;
            ...
            return yield (_bluebirdLstC2 || _load_bluebirdLstC2()).default.map(publishConfigs, function (it) {
                return (0, (_publisher || _load_publisher()).getResolvedPublishConfig)(_this8.info, it, true);
            });
        })();
    }
}
...
function getPublishConfigs(packager, targetSpecificOptions) {
    let publishers;
    ...
    return (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(publishers).map(it => typeof it === "string" ? { provider: it } : it);
}