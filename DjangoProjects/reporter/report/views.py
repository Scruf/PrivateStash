...
def report(request,email):
	...
	some_set_data = ""
	#@seome_set_array will hold the the entrire structed set
	some_set_array = []
	#iterating over the data set 
	for d in data.get_set():
		#iterating over a tupple data . Typples comes from the upper for loop
		for tup in d:
			#concataning the string with a tuple data
			some_set_data =  some_set_data +" "+ str(tup)
			#appending tupple data to array 
		some_set_array.append(some_set_data)
		#reseting some data to empty string 
		some_set_data=""
		#creating context object which will be rendered on the web page
	context = {
		'list': some_set_array
	}
